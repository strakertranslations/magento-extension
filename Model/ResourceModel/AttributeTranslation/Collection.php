    ...
    function addCategoryName( $sourceStoreId = 0 ){
        ...
        if( $sourceStoreId == 0 ){
            $this->getSelect()
                ->joinLeft(
                    ['cn'=> $categoryTable],
                    'main_table.entity_id = cn.entity_id AND cn.store_id = 0 AND cn.attribute_id = 42',
                    ['name' => 'value']
                );
        }else{
            $this->getSelect()
                ->columns(
                    ...
                    'main_table.entity_id = cn_store.entity_id AND cn_store.store_id = ' .$sourceStoreId . ' AND cn_store.attribute_id = 42',
                    []
                )->joinLeft(
                    ['cn_default'=> $categoryTable],
                    'main_table.entity_id = cn_default.entity_id AND cn_default.store_id = 0 AND cn_default.attribute_id = 42',
                    []
                );
        }
